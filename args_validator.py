...
def validate(argv, args, CONFIG_FILE):
    if _args_count_wrong(argv) or _order_not_present(args) or _wrong_order_with_num_servs(argv, args) or _wrong_num_servs(args) or _missing_required_config_file(args, CONFIG_FILE):
        log_error("Los parámetros introducidos al script son incorrectos, ejecutelo con --help para encontrar más información")
        raise ValueError()

def args_warnings(argv, args):
    if args.orden == 'prepare' and len(argv) == 2:
        log_warn("Se usarán 3 servidores como valor predeterminado por la ausencia de este parámetro")
...
def _order_not_present(args):    
    if not args.orden:
        log_error("Se debe especificar la orden a realizar. Vuelva a ejecutar el script con una de las órdenes proporcionadas")
        return True 
